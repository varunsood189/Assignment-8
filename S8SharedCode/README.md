...
├── .env.example       ← copy to .env locally (gitignored); never commit .env
├── .gitignore
│
...
│   ├── visualize_graph.py ← HTML DAG viewer for graph.json
│   ├── scripts/run_assignment.sh ← run parts 1–5 (./scripts/run_assignment.sh 2 = parallel fan-out)
...
```bash
# 1. Secrets (local only — see ../SECURITY.md)
cp .env.example .env
$EDITOR .env                  # add the keys you have
git check-ignore -v .env      # should match .gitignore
...
Visualize the same session as an interactive DAG (opens in your browser):

```bash
uv run python visualize_graph.py <sid> --open
# HTML written to state/sessions/<sid>/graph_view.html
```
