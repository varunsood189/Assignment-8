...
def _last_tool_content(messages: list[dict]) -> str:
    """Gemini often ends the tool loop with empty `text` even after a good
    fetch. Downstream skills then see `{}`. Recover the last tool payload."""
        ...
        async with ClientSession(read, write) as mcp:
            ...
            for _ in range(MAX_TOOL_HOPS + 1):
                ...
                if not tool_calls:
                    ...
                    if not text and any(m.get("role") == "tool" for m in messages):
                        messages.append({
                            "role": "user",
                            "content": (
                                "Using the tool results above, emit ONLY the "
                                "JSON object required by your skill prompt. "
                        ...
                        if text:
                            return synth
                        recovered = _last_tool_content(messages)
                        if recovered:
                            reply = dict(reply)
...
async def _chat(*, messages, tools, agent, session_id, provider_pin,
    ...
    return await _a.to_thread(LLM().chat, **kwargs)
